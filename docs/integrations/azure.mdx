...
<Note>
OBO features require the `azure` extra for MSAL support:
...
For advanced OBO scenarios or when you need more control over the token exchange, use the `MSALApp` dependency to get a pre-configured MSAL `ConfidentialClientApplication`.

```python
from fastmcp import FastMCP
from fastmcp.server.auth.providers.azure import AzureProvider, MSALApp
...
auth_provider = AzureProvider(
    ...
    base_url="http://localhost:8000",
    required_scopes=["mcp-access"],
    additional_authorize_scopes=["https://graph.microsoft.com/.default"],
)

mcp = FastMCP(name="Custom OBO Server", auth=auth_provider)

@mcp.tool
async def custom_graph_call(
    token: AccessToken = CurrentAccessToken,
    msal: ConfidentialClientApplication = MSALApp,
) -> dict:
    """Perform a custom OBO exchange with error handling."""
    ...
    if "error" in result:
        ...
        raise RuntimeError(f"OBO exchange failed: {error_desc}")

    graph_token = result["access_token"]
    # Use graph_token for API calls...
    return {"status": "success", "token_type": result.get("token_type")}
```

The `MSALApp` dependency provides a `ConfidentialClientApplication` configured with your Azure credentials. Unlike `EntraOBOToken`, you're responsible for calling `acquire_token_on_behalf_of` and handling errors yourself.

<Note>
Use `MSALApp` when you need to handle token caching across multiple calls, need detailed error information, or want to use other MSAL features like silent token acquisition.
</Note>
