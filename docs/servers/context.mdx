...
<Note>
Context is accessed through FastMCP's dependency injection system. For other injectable values like HTTP requests, access tokens, and custom dependencies, see [Dependencies](/servers/dependencies).
</Note>
...
For HTTP request access that works regardless of MCP session availability (when using HTTP transports), use the [HTTP request helpers](/servers/dependencies#http-requests) like `get_http_request()` and `get_http_headers()`.
