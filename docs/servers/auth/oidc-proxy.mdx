...
See the [OAuth Proxy CIMD documentation](/servers/auth/oauth-proxy#cimd-support) for complete details on how CIMD works, including trust policies, private key JWT authentication, and security considerations.
...
<ParamField body="cimd_trust_policy" type="CIMDTrustPolicy | None">
  Controls trusted and blocked domains for CIMD clients.
